...
const LogOutButton = ({ classList, userLogOut, template, label, notificationService }) => {
  ...
  const handleClick = () => {
    navigate('/');
    notificationService.notify( t('interface.messages.logOutSuccess') );
    userLogOut();
  }
  ...
  return (
    <>
      {
        template === 'button'
          ? <button className={'button-log-out' + (classList ? ' '+classList : '')} onClick={handleClick}>
              <svg fill="#000000" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 471.2 471.2">
                <g>
                  <g>
          ...
          : <button className="link" onClick={handleClick}>
              { label ?? t('interface.buttons.logOut') }
            </button>
      }
    </>
  );
